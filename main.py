...
def main(page: ft.Page):
    ...
    # Розміри для ПК-версії, Android їх просто проігнорує
    ...
    def add_to_log(e):
        ...
        try:
            ...
            snack = ft.SnackBar(ft.Text("Розрахунок записано в лог!"))
            page.overlay.append(snack)
            snack.open = True
            page.update()
        except:
            page.client_storage.set(f"log_{datetime.now().timestamp()}", f"{txt_pib.value} | {txt_dur.value} дн.")
            snack = ft.SnackBar(ft.Text("Збережено в пам'ять додатку!"))
            page.overlay.append(snack)
            snack.open = True
            page.update()
    ...
    lbl_first = ft.Text("Поverнення в частину: --", size=14, weight="bold", color="#C5C1AA")
        ...
        ft.Container(
            content=ft.Column([
                ...
                ft.Row([
                    ...
                    ft.ElevatedButton("Очистити форму", on_click=close_app, bgcolor="#768B6E", color="#FFFFFF")
                ], alignment="center")
            ], spacing=12),
